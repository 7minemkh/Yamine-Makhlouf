  ...
  if (equation.indexOf("%") > -1) {
    ...
    if (equation != "") {
      if (checkParentheses(equation) == true) {
        ...
        input.value = result;
      } else input.value = "Unmatched Parentheses";
    }
    //input.value = eval(equation);
  }

  if (input.value.indexOf(".") > -1) decimalAdded = true;
  else decimalAdded = false;
}
...
function shuntingYardAlgorithm(expression) {
  ...
  for (var i = 0; i < res1.length; i++) {
    ...
    if (operators.indexOf(token) == -1 && token != "(" && token != ")")
      ...
      operator2 = s.peek();
      while (
        operators.indexOf(operator2) > -1 &&
        ((associative[operator2] == "Left" &&
          precedence[operator1] <= precedence[operator2]) ||
          (associative[operator2] == "Right" &&
            precedence[operator1] < precedence[operator2]))
      ) {
        postfix += operator2 + " ";
        s.pop();
